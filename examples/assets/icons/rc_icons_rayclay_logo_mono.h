#ifndef RC_ICON_RAY_CLAY_LOGO_MONO_H
#define RC_ICON_RAY_CLAY_LOGO_MONO_H

#include "rc_icons_common.h"

/*
    Generated from rayclay-logo-mono.svg. The layout pass owns placement; the
    library draws the icon through the rc_gfx seam during RayClay's custom render pass. Source viewBox: 731.429x731.429.
*/
static inline void rcDrawIconRayClayLogoMono(RC_BoundingBox bounds,
                                             RC_Color color,
                                             const void *userData) {
    (void)userData;
    const float viewBox = 731.429f;

    static const RC_IconPoint path0[] = {
        { 174.754f, 339.714f },
        { 172.722f, 346.167f },
        { 170.945f, 352.684f },
        { 169.425f, 359.256f },
        { 168.164f, 365.874f },
        { 81.714f, 316.714f },
        { 166.434f, 410.344f },
        { 170.28f, 433.965f },
        { 177.384f, 456.931f },
        { 187.628f, 478.858f },
        { 200.842f, 499.382f },
        { 216.803f, 518.16f },
        { 235.248f, 534.878f },
        { 255.868f, 549.259f },
        { 278.319f, 561.063f },
        { 302.227f, 570.092f },
        { 327.193f, 576.197f },
        { 352.802f, 579.275f },
        { 378.626f, 579.275f },
        { 404.235f, 576.197f },
        { 429.202f, 570.092f },
        { 453.11f, 561.063f },
        { 475.561f, 549.259f },
        { 496.181f, 534.878f },
        { 514.625f, 518.16f },
        { 530.587f, 499.382f },
        { 543.8f, 478.858f },
        { 554.044f, 456.931f },
        { 561.148f, 433.965f },
        { 564.994f, 410.344f },
        { 649.714f, 316.714f },
        { 563.264f, 365.874f },
        { 562.003f, 359.256f },
        { 560.483f, 352.684f },
        { 558.706f, 346.167f },
        { 556.674f, 339.714f }
    };
    rcIconDrawPolyline(bounds, path0,
                       (int)(sizeof(path0) / sizeof(path0[0])),
                       viewBox, 34.0f, false, color);

    static const RC_IconPoint path1[] = {
        { 174.754f, 339.714f },
        { 198.624f, 345.925f },
        { 222.494f, 351.308f },
        { 246.364f, 355.863f },
        { 270.234f, 359.589f },
        { 294.104f, 362.488f },
        { 317.974f, 364.558f },
        { 341.844f, 365.8f },
        { 365.714f, 366.214f },
        { 389.584f, 365.8f },
        { 413.454f, 364.558f },
        { 437.324f, 362.488f },
        { 461.194f, 359.589f },
        { 485.064f, 355.863f },
        { 508.934f, 351.308f },
        { 532.804f, 345.925f },
        { 556.674f, 339.714f }
    };
    rcIconDrawPolyline(bounds, path1,
                       (int)(sizeof(path1) / sizeof(path1[0])),
                       viewBox, 34.0f, false, color);

    static const RC_IconPoint path2[] = {
        { 174.754f, 339.714f },
        { 170.461f, 334.096f },
        { 165.532f, 327.439f },
        { 160.056f, 319.853f },
        { 154.122f, 311.449f },
        { 147.819f, 302.335f },
        { 141.238f, 292.623f },
        { 134.466f, 282.421f },
        { 127.594f, 271.839f },
        { 120.711f, 260.988f },
        { 113.906f, 249.978f },
        { 107.268f, 238.918f },
        { 100.887f, 227.917f },
        { 94.852f, 217.087f },
        { 89.251f, 206.537f },
        { 84.176f, 196.376f },
        { 79.714f, 186.714f },
        { 96.198f, 181.445f },
        { 113.988f, 176.9f },
        { 132.844f, 173.102f },
        { 152.527f, 170.074f },
        { 172.795f, 167.836f },
        { 193.41f, 166.412f },
        { 214.129f, 165.822f },
        { 234.714f, 166.089f },
        { 254.924f, 167.236f },
        { 274.519f, 169.283f },
        { 293.258f, 172.253f },
        { 310.902f, 176.167f },
        { 327.209f, 181.049f },
        { 341.941f, 186.919f },
        { 354.856f, 193.8f },
        { 365.714f, 201.714f }
    };
    rcIconDrawPolyline(bounds, path2,
                       (int)(sizeof(path2) / sizeof(path2[0])),
                       viewBox, 34.0f, false, color);

    static const RC_IconPoint path3[] = {
        { 556.674f, 339.714f },
        { 560.967f, 334.096f },
        { 565.896f, 327.439f },
        { 571.373f, 319.853f },
        { 577.307f, 311.449f },
        { 583.609f, 302.335f },
        { 590.191f, 292.623f },
        { 596.962f, 282.421f },
        { 603.834f, 271.839f },
        { 610.718f, 260.988f },
        { 617.523f, 249.978f },
        { 624.161f, 238.918f },
        { 630.542f, 227.917f },
        { 636.577f, 217.087f },
        { 642.177f, 206.537f },
        { 647.253f, 196.376f },
        { 651.714f, 186.714f },
        { 635.231f, 181.445f },
        { 617.441f, 176.9f },
        { 598.584f, 173.102f },
        { 578.902f, 170.074f },
        { 558.633f, 167.836f },
        { 538.019f, 166.412f },
        { 517.299f, 165.822f },
        { 496.714f, 166.089f },
        { 476.504f, 167.236f },
        { 456.91f, 169.283f },
        { 438.17f, 172.253f },
        { 420.527f, 176.167f },
        { 404.219f, 181.049f },
        { 389.488f, 186.919f },
        { 376.573f, 193.8f },
        { 365.714f, 201.714f }
    };
    rcIconDrawPolyline(bounds, path3,
                       (int)(sizeof(path3) / sizeof(path3[0])),
                       viewBox, 34.0f, false, color);

    static const RC_IconPoint path4[] = {
        { 248.714f, 509.714f },
        { 263.402f, 524.285f },
        { 278.214f, 536.746f },
        { 293.152f, 547.097f },
        { 308.214f, 555.339f },
        { 323.402f, 561.472f },
        { 338.714f, 565.496f },
        { 354.152f, 567.41f },
        { 369.714f, 567.214f },
        { 385.402f, 564.91f },
        { 401.214f, 560.496f },
        { 417.152f, 553.972f },
        { 433.214f, 545.339f },
        { 449.402f, 534.597f },
        { 465.714f, 521.746f },
        { 482.152f, 506.785f },
        { 498.714f, 489.714f }
    };
    rcIconDrawPolyline(bounds, path4,
                       (int)(sizeof(path4) / sizeof(path4[0])),
                       viewBox, 34.0f, false, color);

    rcIconDrawCircleStroke(bounds, 73.714f, 178.714f, 30.0f, viewBox, 2.0f, color);
    rcIconDrawCircleStroke(bounds, 657.714f, 178.714f, 30.0f, viewBox, 2.0f, color);
    rcIconDrawCircleStroke(bounds, 307.714f, 444.714f, 27.0f, viewBox, 2.0f, color);
    rcIconDrawCircleStroke(bounds, 457.714f, 444.714f, 27.0f, viewBox, 2.0f, color);
}

static inline void rcIconRayClayLogoMono(float size, RC_Color color) {
    rcIconEmit(size, color, rcDrawIconRayClayLogoMono, NULL);
}

#endif /* RC_ICON_RAY_CLAY_LOGO_MONO_H */
