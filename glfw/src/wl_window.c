...
// ===========================================================================
// RAYCLAY PATCH - nobar Wayland interactive move/resize
// ---------------------------------------------------------------------------
// WARN RE-APPLY THIS ON EVERY GLFW BUMP. Rebased 3.4.0 -> 3.5.1 on 2026-08-02.
// Grep `RAYCLAY PATCH` across the vendored tree to find every such block.
//
// GLFW uses xdg_toplevel interactive move/resize internally for its own
// fallback decorations but never exports it. nobar draws RayClay's title bar
// itself and has no wl_seat or input serial of its own, so it delegates drag
// and edge-resize to these two entry points. `edge` is passed straight through
// to xdg_toplevel_resize, so the caller supplies an XDG_TOPLEVEL_RESIZE_EDGE_*
// value. Both no-op off Wayland.
//
// WARN THESE TWO SIGNATURES ARE A PUBLISHED CONTRACT, not a private helper.
// The title-bar layer declares both `extern` and stubs them in its own
// self-test, so changing a SIGNATURE breaks that build rather than this one.
//
// WARN 3.5.1 REBASE NOTE: GLFW 3.5 added window->wl.fallback.buttonPressSerial and
// uses it for its OWN fallback decorations. That is not a replacement for
// _glfw.wl.serial, which is still assigned at seven input sites exactly as in
// 3.4 and is the correct serial here - nobar calls in from its own hit test,
// not from a GLFW fallback button press.
//
// The xdg_toplevel that owns interactive move/resize. Under libdecor (GNOME/
// Mutter) GLFW drives the window through a libdecor frame and leaves its own
// window->wl.xdg.toplevel NULL - the real toplevel lives inside the frame, so
// fall back to it. Returns NULL if neither is available. Caller must already
// have confirmed the Wayland platform (the wl.* fields are only valid there).
static struct xdg_toplevel* _glfwWaylandMoveResizeToplevel(_GLFWwindow* window)
{
    if (window->wl.xdg.toplevel)
...
GLFWAPI void glfwWaylandStartWindowMove(GLFWwindow* handle)
{
    _GLFWwindow* window = (_GLFWwindow*) handle;
    if (!window || _glfw.platform.platformID != GLFW_PLATFORM_WAYLAND || !_glfw.wl.seat)
        return;
    struct xdg_toplevel* toplevel = _glfwWaylandMoveResizeToplevel(window);
    if (toplevel)
        xdg_toplevel_move(toplevel, _glfw.wl.seat, _glfw.wl.serial);
}

GLFWAPI void glfwWaylandStartWindowResize(GLFWwindow* handle, int edge)
{
    _GLFWwindow* window = (_GLFWwindow*) handle;
    if (!window || _glfw.platform.platformID != GLFW_PLATFORM_WAYLAND || !_glfw.wl.seat)
        return;
    struct xdg_toplevel* toplevel = _glfwWaylandMoveResizeToplevel(window);
    if (toplevel)
        xdg_toplevel_resize(toplevel, _glfw.wl.seat, _glfw.wl.serial, (uint32_t) edge);
}
// ============================ end RAYCLAY PATCH ============================
